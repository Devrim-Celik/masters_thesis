...
class SourceAS(AutonomousSystem):
	...
	:param atk_path_signal: whether the atk_path_singal was send out and is currently in affect
	:param attack_traffic_recording: records the send out attack packets for later plotting
	...
	def attack_cycle(self):
		...
		while True:
			...
			if self.env.now < self.attack_start:
				...
				attack_volume = self.full_attack_vol - (self.env.now - (self.attack_slowdown + self.attack_stop + 150)) * ((self.full_attack_vol - self.standard_load) / (self.attack_stop - self.attack_slowdown)) + random.randint(-10, 10)
			else:
				attack_volume = random.randint(self.standard_load - 10, self.standard_load + 10)
