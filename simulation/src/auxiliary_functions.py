"""
Contains various auxiliary function, among other things, function to load/save
files and to create/display figures.
...
''' Currently not used
def save_as_pickle(object_to_pickle, path_to_pickle, verbose=False):
	"""
...
def gen_pyvis_network(Graph):
	...
	net = Network(notebook=True, directed=True, height='1000px',
				  width="100%")
...
def save_pyvis_network(Graph, html_path, verbose=False):
	"""
...
def assign_attributes(Graph, victim, adversary, allies=[],
					  type_to_value={"T": 30, "M": 20, "C": 10, "CP": 10}):
	"""
	Given a graph, the victim and adversary node (and optionally a set of
	helpers), this function will assign an array of attributes to these nodes.

	:param Graph: the graph for whose nodes and edges attributes will be
	    assigned
	...
	:param type_to_value: a dictionary, cotaining value attributes according
	    to the type of node
